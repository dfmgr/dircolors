...
# @@File             :  install.sh
# @@Description      :  Install configurations for dircolors
...
__run_post_install() {
  local getRunStatus=0
  [ -L "$HOME/.dircolors" ] || __symlink "$APPDIR/default" "$HOME/.dircolors"
  return $getRunStatus
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
